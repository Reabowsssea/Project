...
def process_images(rgb_folder, depth_folder, output_folder, num_sparse_points, csv_output):
    ...
    for rgb_filename in os.listdir(rgb_folder):
        ...
        # 保存稀疏深度图
        sparse_depth_filename = f"sparse_{rgb_filename}"
        sparse_depth_path = os.path.join(output_folder, sparse_depth_filename)
        cv2.imwrite(sparse_depth_path, sparse_depth)

        # 将数据加入 CSV
        csv_data.append([rgb_path, depth_path, sparse_depth_path])

    # 将数据保存到 CSV 文件
    df = pd.DataFrame(csv_data, columns=["RGB_Image", "Depth_Image", "Sparse_Depth_Image"])
...
if __name__ == "__main__":
    ...
    output_folder = "/root/autodl-tmp/UWAmodel_2/datasets/data_splits_sds"
