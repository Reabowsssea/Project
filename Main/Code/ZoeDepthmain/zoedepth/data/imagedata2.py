...
import os
from ZoeDepthmain.zoedepth.utils.depth_seg import extract_points
from ZoeDepthmain.zoedepth.utils.config import change_dataset
# from .preprocess import CropParams, get_white_border, get_black_border
import csv
from ZoeDepthmain.zoedepth.utils.depth_seg import extract_points
from ZoeDepthmain.zoedepth.utils.depth_prior import generate_row_col_depth
from ZoeDepthmain.zoedepth.data.NNfill import fill_in_fast
...
class DataLoadPreprocess(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        sample = {'image': image,  'focal': focal, 'sparse_depth': sds,  'has_valid_depth': has_valid_depth, 'image_path': img_path,'mask': mask, **sample}
        ...
        sample = {**sample, 'image_path': os.path.basename(img_path)}
...
class ToTensor(object):
    ...
    def __call__(self, sample):
        ...
        if self.mode == 'train':
            ...
            return {**sample, 'image': image,  'sparse_depth': sds,'focal': focal}
        else:
            has_valid_depth = sample['has_valid_depth']
            image = self.resize(image)
            # return {**sample, 'image': image, 'depth': depth, 'sparse_depth': sds, 'focal': focal, 'has_valid_depth': has_valid_depth,
            #         'image_path': sample['image_path'], 'depth_path': sample['depth_path']}
            return {**sample, 'image': image,  'focal': focal, 'sparse_depth': sds,  'has_valid_depth': has_valid_depth}
