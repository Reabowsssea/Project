...
def random_coordinate_in_region(coordinates, region_top_left=(60,200), region_bottom_right=(420,560)):
    """
    ...
    if valid_coordinates:
        ...
        return np.array(selected_coordinate).reshape(1, 2)
    else:
        return None  # 如果指定区域内没有符合条件的坐标，则返回 None


def generate_row_col_depth(pts_depth, depth_gt):
    # depth_gt = np.transpose(depth_gt, (2, 1, 0))
    depth_gt = depth_gt.squeeze()
    row_col_depth_list = []
    # for i in range(len(depths)):
    # depth_gt = depth_gt.squeeze()
    # print(pts_depth[:, 0].round(), pts_depth[:, 1].round())
    if np.any(pts_depth[:, 0].round() < 0) or np.any(pts_depth[:, 0].round() >= 480) or \
        np.any(pts_depth[:, 1].round() < 0) or np.any(pts_depth[:, 1].round() >= 640):
        print(pts_depth)
    depth_values = depth_gt[pts_depth[:, 0].round(), pts_depth[:, 1].round()]
    ...
    # depth_values_depth = depth_values_depth.to('cuda')

    row_col_depth = np.concatenate((pts_depth, depth_values), axis=1)
    ...
    # depth_samples = np.concatenate(row_col_depth_list, axis=0)
    # depth_samples = torch.from_numpy(combined_row_col_depth).to("cpu")
    return row_col_depth
