...
def update_sample(bin_edges, target_bin_left, target_bin_right, depth_r, min_depth, max_depth, uncertainty_range):
    ...
    with torch.no_grad():    
        ...
        interval = depth_range / 64
        interval = interval.repeat(1, 64, 1, 1)
        # bin_edges = torch.cat([torch.ones([b, 1, h, w], device=bin_edges.device) * depth_start_update, bin_edges[:, 1:, ...]], 1)
        bin_edges = torch.cat([torch.ones([b, 1, h, w], device=bin_edges.device) * depth_start_update, interval], 1)

        bin_edges = torch.cumsum(bin_edges, 1).clamp(min_depth, max_depth)
        curr_depth = 0.5 * (bin_edges[:, :-1, ...] + bin_edges[:, 1:, ...])
        if torch.isnan(curr_depth).any():
