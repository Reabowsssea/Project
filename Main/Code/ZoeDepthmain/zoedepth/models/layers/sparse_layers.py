...
class PMP(nn.Module):
    ...
    def __init__(self, level, in_ch, out_ch, up=True, pool=True):
        super().__init__()
        self.level = level
        if up:
    ...
    def pinv(self, S, xx, yy):
        ...
        # Py = S * (yy - cy) / fy
        # Pz = S
        Pxyz = torch.cat([xx, yy, S], dim=1).contiguous()
        return Pxyz

    def forward(self, fout, dout, XI, S):
        ...
        #                         indexing='xy')
        ###############################################################
        # Pre fout是rgb特征，dout是深度图输出
        # Pxyz = self.pinv(Sp, xx, yy)
        # Ofnum, args = self.dist(Sp, xx, yy)
        # dout = self.prop(fout, Pxyz, Ofnum, args)
        ###############################################################
        # MF
        # Pxyz = self.pinv(dout, xx, yy).view(dout.shape[0], 3, dout.shape[2], dout.shape[3])
...
class SparsePerceptionPool(nn.Module):
    def __init__(self, in_channels, out_channels):
        super(SparsePerceptionPool, self).__init__()
        self.conv1x1 = nn.Conv2d(in_channels, out_channels, kernel_size=1)
        # self.conv1x64 = nn.Conv2d(in_channels, 64, kernel_size=1)
        self.downsamplers = self._make_downsampler() 
        self.downsamplers = nn.ModuleList([
            self._make_downsampler(128),  # 下采样至 128x128
            self._make_downsampler(64),   # 下采样至 64x64
            self._make_downsampler(32)    # 下采样至 32x32
        ])

    def _make_downsampler(self, level):
        return nn.Sequential(
            nn.AvgPool2d(kernel_size=3, stride=2, padding=1),  # 步长2的3x3平均池化
            nn.Conv2d(1, 1, kernel_size=1),  # 1x1卷积
    ...
    def forward(self, x):
        ...
        for downsampler in self.downsamplers:
            x = downsampler(x.float())
            outputs.append(x)
        return outputs
    
def merge_sparse_into_output(sparse_tensor, output_tensor):
    """
    Merge non-zero values from sparse_tensor into output_tensor.

    Args:
        sparse_tensor (torch.Tensor): Sparse tensor.
        output_tensor (torch.Tensor): Output tensor.

    Returns:
        torch.Tensor: Output tensor with non-zero values from sparse tensor merged.
    """
    # Find non-zero indices in sparse tensor
    nonzero_indices = torch.nonzero(sparse_tensor > 0).to('cuda')
    ...
    # Check if any non-zero indices were found
    if nonzero_indices.size(0) == 0:
        return output_tensor, nonzero_indices, torch.tensor([]).to('cuda')

    # Extract non-zero values from sparse tensor
    sparse_values = sparse_tensor[nonzero_indices[:, 0], 
                                   nonzero_indices[:, 1], 
                                   nonzero_indices[:, 2], 
                                   nonzero_indices[:, 3]]

    # Ensure nonzero_indices are within the bounds of output_tensor
    max_indices = output_tensor.shape
    valid_mask = (nonzero_indices[:, 0] < max_indices[0]) & \
                 (nonzero_indices[:, 1] < max_indices[1]) & \
                 (nonzero_indices[:, 2] < max_indices[2]) & \
                 (nonzero_indices[:, 3] < max_indices[3])

    # Get valid indices
    valid_nonzero_indices = nonzero_indices[valid_mask]
    sparse_values = sparse_values[valid_mask]

    # Debugging: Check valid indices
    # print(f"Valid nonzero_indices shape: {valid_nonzero_indices.shape}")

    # Ensure there are valid indices before assigning
    if valid_nonzero_indices.size(0) > 0:
        output_tensor[valid_nonzero_indices[:, 0], 
                      valid_nonzero_indices[:, 1], 
                      valid_nonzero_indices[:, 2], 
                      valid_nonzero_indices[:, 3]] = sparse_values

    return output_tensor, valid_nonzero_indices, sparse_values
...
class FeatureExtractor(nn.Module):
    def __init__(self):
        ...
        self.pool5 = nn.AvgPool2d(kernel_size=2, stride=2, padding=1)
