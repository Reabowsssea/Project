...
class MultiStepPredictor(nn.Module):
    def __init__(self, steps=3):
        ...
        self.decoder3 = nn.ConvTranspose2d(32 + 32, 3 * steps, kernel_size=2, stride=2)  # 输出原始尺寸
    ...
    def forward(self, x):
        ...
        return self.sigmoid(d3).view(x.size(0), self.steps, 3, x.size(2), x.size(3))
...
def generate_diff_image(input_img, model, transform):
    ...
    # save_dir= "/root/autodl-tmp/ceshi/diff"
...
# def parse_args():
#     """解析命令行参数"""
#     parser = argparse.ArgumentParser(description="Generate Diff Image from Input Image")
...
#     # 生成差分图像
#     generate_diff_image(args.image_path, model, transform, output_dir=args.output_dir)
