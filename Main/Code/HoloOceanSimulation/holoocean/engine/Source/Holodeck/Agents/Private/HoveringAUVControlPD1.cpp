#include "Holodeck.h"
#include "HoveringAUVControlPD1.h"


UHoveringAUVControlPD1::UHoveringAUVControlPD1(const FObjectInitializer& ObjectInitializer) :
		Super(ObjectInitializer), PositionController(AUV_POS_P, 0, AUV_POS_D), RotationController(AUV_ROT_P, 0, AUV_ROT_D) { }

void UHoveringAUVControlPD1::Execute(void* const CommandArray, void* const InputCommand, float DeltaSeconds) {
	if (HoveringAUV == nullptr) {
		HoveringAUV = static_cast<AHoveringAUV*>(HoveringAUVController->GetPawn());
		if (HoveringAUV == nullptr) {
			UE_LOG(LogHolodeck, Error, TEXT("UHoveringAUVControlPD1 couldn't get HoveringAUV reference"));
			return;
		}
