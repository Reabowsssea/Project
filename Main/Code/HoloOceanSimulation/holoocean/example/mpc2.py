...
# 加载预训练的光流模型 (RAFT)
def load_flow_model():
    model = optical_flow.raft_large(pretrained=True, progress=True)
    model = model.eval()
    return model
...
def calculate_optical_flow_deep(model, prev_frame, current_frame):
    ...
    # 预测光流
    with torch.no_grad():
        flow = model(prev_tensor, curr_tensor)[-1]
    ...
    with holoocean.make(scenario) as env:
        while True:
            ...
            if "PoseSensor" in state:
                ...
                current_position = pose[:3]  # 提取位置信息 (x, y, z)
            ...
            if "LeftCamera" in state:
                ...
                if current_position is not None:
                    current_position = np.ravel(current_position)
                    pos_text = (
                        f"Position: x={current_position[0]:.2f}, "
                        f"y={current_position[1]:.2f}, "
                        f"z={current_position[2]:.2f}"
                    )
                    cv2.putText(display_frame, pos_text, (20, 80),
                                cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2)

                # 显示按键提示
                help_text = "i:前进 k:后退 j:左转 l:右转 w:上浮 s:下潜 a:左移 d:右移 m:MPC h:手动 q:退出"
                cv2.putText(display_frame, help_text, (20, display_frame.shape[0] - 20),
