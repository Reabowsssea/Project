...
class SegmentationDataset(Dataset):
	...
	def __getitem__(self, idx):
		...
		import cv2
		# 加载差分图
		# im2 = Image.open(file2_path).convert('L')  # 确保差分图是灰度图
		# im2 = np.array(im2)
		# im2 = cv2.resize(im2, (256, 256))  # 调整大小
		# im2 = im2 / 255.0  # 归一化
		# im2 = np.expand_dims(im2, axis=0)  # 添加通道维度

		im21 = Image.open(file2_path).convert('L')  # 确保差分图是灰度图
		im21 = np.array(im21)
		im21 = cv2.resize(im21, (256, 256))  # 调整大小
		im21 = im21 / 255.0  # 归一化
		im21 = np.expand_dims(im21, axis=0)  # 添加通道维度
		...
		sample = {'image': merged_array, 'diff': im21,'labeled': lbl}
